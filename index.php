...
switch ($action) {
	...
	try {
	    ...
	    $message = 'Please enter the date in valid format.';
	    break;
	    }
	...
	$current_date_f = $current_date_o->format($format_string);
        ...
        $time_dur = $current_date_o->diff($due_date_o);
	if ($due_date_o < $current_date_o) {
	    $due_date_message = $time_dur->format('This invoice is %y years, %m months, and %d days overdue.');
            } else {
            $due_date_message = $time_dur->format('This invoice is due in %y years, %m months, and %d days.');
            }
